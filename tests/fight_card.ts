...
describe.only("Create fight card", () => {
    ...
    before("Initialize", async () => {
        ...
        let fetchedProgram = await program.account.programData.fetch(program_account.publicKey);
        const [event_account_one]  = anchor.web3.PublicKey.findProgramAddressSync(
            [
    ...
    it("Should add a new fight card", async () => {
        ...
        const [fight_card_account_one, event_account_one_bump] = anchor.web3.PublicKey.findProgramAddressSync(
            [
                Buffer.from("BattleBoosters"),
        ...
        const tx = await program.methods.createNewFightCard(fightCardData).accounts({
            creator: admin_account.publicKey,
            program: program_account.publicKey,
            event: event_account,
        ...
        let fetchedFightCard = await program.account.fightCardData.fetch(fight_card_account_one);
        ...
        assert.deepEqual(fetchedFightCard.eventPubkey.equals(event_account), true);
    ...
    it("Should update fight card", async () => {
        ...
        const tx = await program.methods.updateFightCard(0, fightCardData).accounts({
            creator: admin_account.publicKey,
            program: program_account.publicKey,
            event: event_account,
        ...
        let fetchedFightCard = await program.account.fightCardData.fetch(fight_card_account);
        assert.equal(fetchedFightCard.id.eq(new BN(112)), true );
        assert.equal(fetchedFightCard.fightDuration, null);
